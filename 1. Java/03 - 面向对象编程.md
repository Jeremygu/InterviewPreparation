...
Java中每一个类都有构造函数，当使用关键字new实例化一个对象，类的构造函数就会被自动调用，完成对象的初始化工作。
...
>
> (4) 一个类可以定义多个构造函数，如果在定义类时没有定义构造函数，则编译系统会自动插入一个无参数的默认构造函数，这个构造器不执行任何代码；
> 
...
(3) 初始化父类中的普通成员变量和代码块(按照在程序中出现的顺序初始化)，然后再执行父类中的构造方法

(4) 初始化子类中的普通成员变量和代码块(按照在程序中出现的顺序初始化)，然后再执行子类中的构造方法
...
(1) final修饰的变量的值不可被修改，方法不可被重写，类不可被继承。

(2) 形式参数只能用final修饰符，任何其它修饰符都会引起编译错误。但是用这个修饰符也有一定的限制，就是在方法中不能对参数做任何修改。不过在一般情况下，一个方法的形参不用final修饰。只有在特殊情况下：方法内部类，一个方法内的内部类如果使用了这个方法的参数或者局部变量，这个参数或局部变量应该是final。
...
(1) 使用super()或this()时，它们必须放在构造方法的第一行；
...
() 抽象类中的成员方法的访问类型可以为public、protected和private；而接口中的成员方法的访问类型默认为public abstract类型，且只能是public
